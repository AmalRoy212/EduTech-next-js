...
interface PlanetVariantsOptions {
...
export const slideIn = (
  ...
  show: { x: number; y: number; transition: { type: string; delay: number; duration: number; ease: string } };
} => ({
  hidden: {
...
export const textContainer: {
  ...
  show: (i?: number) => { opacity: number; transition: { staggerChildren: number; delayChildren: number } };
} = {
  hidden: { opacity: 0 },
  show: (i = 1) => ({
    opacity: 1,
...
export const textVariant2: {
  ...
  show: {
    opacity: number;
    y: number;
    transition: { type: string; ease: string };
  };
} = {
